...
var skillsResult = _(dataArray).map('skills').flattenDeep().sortBy().sortedUniq().value(); 
console.log('Массив скилов: ', skillsResult);

var nameResult = _(dataArray).map('name').sortBy('friends').value();
console.log('Массив имен: ', nameResult);

var friendsResult = _(dataArray).map('friends').flatten().map('name').uniq(dataArray).value();
